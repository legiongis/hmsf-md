...
from arches.app.models.models import Resource2ResourceConstraint
from arches.app.models.graph import Graph
from arches.app.models.card import Card
from fpan.utils.fpan_account_creation import (
    ...
    def load_package(
        ...
        def load_graphs(package_dir):
            ...
            management.call_command(
                "packages", operation="import_graphs", source=branches, overwrite=True
            )
            management.call_command(
                "packages",
                operation="import_graphs",
                source=resource_models,
                overwrite=True,
            )

        def load_concepts(package_dir):
            concept_data = glob.glob(
                os.path.join(package_dir, "reference_data", "concepts", "*.xml")
            )
            collection_data = glob.glob(
                os.path.join(package_dir, "reference_data", "collections", "*.xml")
            )
        ...
        def load_permissions(package_dir):
            ...
            for g_name, data in perm_configs.items():
                try:
                    ...
                    if not g.isresource:
                        print(
                            f"graph: {g_name} - Error: this is a Branch, not a Resource Model\n--"
                        )
                        continue
                ...
                for group_name, perms in data.items():
                    ...
                    for perm_type, card_names in perms.items():
                        if perm_type not in valid_perms:
                            print(
                                f"    permission: {perm_type} - Error: invalid permission type"
                            )
                            perm_error = True
                            continue
        ...
        if ("data" in components or components == "all") and exclude_data is not True:
