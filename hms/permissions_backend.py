...
def generate_site_access_html(user):
    """This function should be called by a context_processor to dynamically generate
    HTML content that is used in the rule filter component template. It can be altered
    as needed."""

    ## ultimately, this should be dynamically driven directly by the rules this filter
    # finds. for now though, there is a lot of hard-coded HMS logic

    FULL_ACCESS_HTML = (
        "<p>Your account has full access to <strong>all</strong> "
    ...
    if user_is_scout(user):
        if user.scout.scoutprofile.site_access_mode == "FULL":
            ...
            return (
                "<p>You have access to any archaeological sites to which you have "
                "been individually assigned.</p>"
            )
        else:
            return NO_ACCESS_HTML
    ...
    if user_is_land_manager(user):
        if user.landmanager.site_access_mode == "FULL":
            ...
            html = (
                "<p>You can access any archaeological sites that are located in "
                "the following Management Areas:</p>"
            )
            if len(user.landmanager.all_areas) == 0:
                html += (
                    "<p><em>No Management Areas have been added to your Land "
                    "Manager profile.</em></p>"
                )
            else:
                html += (
            ...
            html = (
                "<p>You can access any archaeological sites that are managed by your "
                f"agency: <strong>{user.landmanager.management_agency}</strong></p>"
            )
        else:
            html = NO_ACCESS_HTML
